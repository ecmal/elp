...
(function (ts) {
    ts.Diagnostics = {
        ...
        Signature_0_must_be_a_type_predicate: { code: 1224, category: ts.DiagnosticCategory.Error, key: "Signature_0_must_be_a_type_predicate_1224", message: "Signature '{0}' must be a type predicate." },
        ...
        String_literal_with_double_quotes_expected: { code: 1327, category: ts.DiagnosticCategory.Error, key: "String_literal_with_double_quotes_expected_1327", message: "String literal with double quotes expected." },
        Property_value_can_only_be_string_literal_numeric_literal_true_false_null_object_literal_or_array_literal: { code: 1328, category: ts.DiagnosticCategory.Error, key: "Property_value_can_only_be_string_literal_numeric_literal_true_false_null_object_literal_or_array_li_1328", message: "Property value can only be string literal, numeric literal, 'true', 'false', 'null', object literal or array literal." },
        ...
        Specify_module_code_generation_Colon_none_commonjs_amd_system_umd_es2015_or_ESNext: { code: 6016, category: ts.DiagnosticCategory.Message, key: "Specify_module_code_generation_Colon_none_commonjs_amd_system_umd_es2015_or_ESNext_6016", message: "Specify module code generation: 'none', commonjs', 'amd', 'system', 'umd', 'es2015', or 'ESNext'." },
...
(function (ts) {
    ...
    function escapeIdentifier(identifier) {
        return escapeLeadingUnderscores(identifier);
    }
    ...
    function entityNameToString(name) {
        switch (name.kind) {
            case 71:
                return getFullWidth(name) === 0 ? ts.unescapeLeadingUnderscores(name.text) : getTextOfNode(name);
            case 143:
    ...
    function getJSDocParameterTags(param) {
        ...
        var tags = getJSDocTags(func);
        if (!tags)
            return undefined;
        if (!param.name) {
            var paramIndex = func.parameters.indexOf(param);
            ts.Debug.assert(paramIndex !== -1);
            var curParamIndex = 0;
            ...
            return tags.filter(function (tag) { return ts.isJSDocParameterTag(tag) && tag.name.text === name_1; });
        }
        else {
            return undefined;
        }
    ...
    function getTextOfIdentifierOrLiteral(node) {
        if (node) {
            if (node.kind === 71) {
                return ts.unescapeLeadingUnderscores(node.text);
            }
            if (node.kind === 9 ||
                node.kind === 8) {
                return node.text;
...
(function (ts) {
    ...
    function unescapeIdentifier(id) {
        return unescapeLeadingUnderscores(id);
    }
...
(function (ts) {
    ...
    function forEachChild(node, cbNode, cbNodes) {
        ...
        switch (node.kind) {
            ...
            case 293:
                return visitNodes(cbNode, cbNodes, node.jsDocPropertyTags);
            case 292:
                return visitNode(cbNode, node.typeExpression) ||
                    visitNode(cbNode, node.name);
            case 297:
    ...
    (function (Parser) {
        ...
        function createIdentifier(isIdentifier, diagnosticMessage) {
            ...
            if (isIdentifier) {
                ...
                node.text = ts.escapeLeadingUnderscores(internIdentifier(scanner.getTokenValue()));
                nextToken();
                return finishNode(node);
            }
        ...
        (function (JSDocParser) {
            ...
            function parseJSDocCommentWorker(start, length) {
                ...
                function createJSDocIdentifier(isIdentifier, createIfMissing) {
                    ...
                    result.text = ts.escapeLeadingUnderscores(content.substring(pos, end));
...
(function (ts) {
    ...
    function readConfigFile(fileName, readFile) {
        ...
        try {
            ...
            return { config: {}, error: ts.createCompilerDiagnostic(ts.Diagnostics.Cannot_read_file_0_Colon_1, fileName, e.message) };
        }
    ...
    function readJsonConfigFile(fileName, readFile) {
        ...
        try {
            ...
            return { parseDiagnostics: [ts.createCompilerDiagnostic(ts.Diagnostics.Cannot_read_file_0_Colon_1, fileName, e.message)] };
        }
        return ts.parseJsonText(fileName, text);
    }
    ...
    function validateSpecs(specs, errors, allowTrailingRecursion, jsonSourceFile, specKey) {
        ...
        for (var _i = 0, specs_1 = specs; _i < specs_1.length; _i++) {
            ...
            if (!allowTrailingRecursion && invalidTrailingRecursionPattern.test(spec)) {
                errors.push(createDiagnostic(ts.Diagnostics.File_specification_cannot_end_in_a_recursive_directory_wildcard_Asterisk_Asterisk_Colon_0, spec));
            }
            else if (invalidMultipleRecursionPatterns.test(spec)) {
                errors.push(createDiagnostic(ts.Diagnostics.File_specification_cannot_contain_multiple_recursive_directory_wildcards_Asterisk_Asterisk_Colon_0, spec));
            }
            else if (invalidDotDotAfterRecursiveWildcardPattern.test(spec)) {
                errors.push(createDiagnostic(ts.Diagnostics.File_specification_cannot_contain_a_parent_directory_that_appears_after_a_recursive_directory_wildcard_Asterisk_Asterisk_Colon_0, spec));
            }
            else {
                ...
                for (var _i = 0, _a = ts.getPropertyAssignment(jsonSourceFile.jsonObject, specKey); _i < _a.length; _i++) {
                    ...
                    if (ts.isArrayLiteralExpression(property.initializer)) {
                        for (var _b = 0, _c = property.initializer.elements; _b < _c.length; _b++) {
                            var element = _c[_b];
                            if (element.kind === 9 && element.text === spec) {
    ...
    (function (JsTyping) {
        ...
        function discoverTypings(host, fileNames, projectRootPath, safeListPath, packageNameToTypingLocation, typeAcquisition, unresolvedImports) {
            ...
            if (!safeList) {
                ...
                safeList = ts.createMapFromTemplate(result.config);
            }
            var filesToWatch = [];
            ts.forEach(typeAcquisition.include, addInferredTyping);
            var exclude = typeAcquisition.exclude || [];
            var possibleSearchDirs = ts.createMap();
            for (var _i = 0, fileNames_2 = fileNames; _i < fileNames_2.length; _i++) {
                var f = fileNames_2[_i];
                possibleSearchDirs.set(ts.getDirectoryPath(f), true);
            }
            ...
            function getTypingNamesFromJson(jsonPath, filesToWatch) {
                ...
                function addInferredTypingsFromKeys(map) {
                    for (var key in map) {
            ...
            function getTypingNamesFromSourceFileNames(fileNames) {
                for (var _i = 0, fileNames_3 = fileNames; _i < fileNames_3.length; _i++) {
                    ...
                    var safe = safeList.get(cleanedTypingName);
                    if (safe !== undefined) {
